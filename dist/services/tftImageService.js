 ...
 */
async function fetchTFTMetadata(type) {
    // Check cache first
    ...
    try {
        ...
        const response = await axios_1.default.get(`https://ddragon.leagueoflegends.com/cdn/${CURRENT_PATCH}/data/en_US/tft-${type}.json`);
        // Cache the response
 ...
 */
async function getTFTChampionInfo(championId) {
    ...
    const possibleIds = Object.keys(metadata.data).filter(id => id.includes(cleanId) || cleanId.includes(id.replace('TFT9_', '')));
 ...
 */
async function getTFTTraitInfo(traitId) {
 ...
 */
async function fetchSpriteImage(spritePath) {
    try {
        const cacheKey = `sprite_${spritePath}`;
        // Check cache first
        if (imageCache[cacheKey]) {
            return await (0, canvas_1.loadImage)(imageCache[cacheKey]);
        }
        // Fetch the sprite image
        const response = await axios_1.default.get(`https://ddragon.leagueoflegends.com/cdn/${CURRENT_PATCH}/img/${spritePath}`, { responseType: 'arraybuffer' });
 ...
 */
function extractIconFromSprite(spriteImage, x, y, width, height) {
 ...
 */
async function fetchTFTChampionIcon(championId) {
    try {
        ...
        if (championInfo && championInfo.image) {
            try {
                ...
                try {
                    const response = await axios_1.default.get(`https://ddragon.leagueoflegends.com/cdn/${CURRENT_PATCH}/img/${fullImagePath}`, { responseType: 'arraybuffer' });
                    const buffer = Buffer.from(response.data);
                    imageCache[cacheKey] = buffer;
                    return await (0, canvas_1.loadImage)(buffer);
                }
                    ...
                    const iconBuffer = extractIconFromSprite(spriteImage, championInfo.image.x, championInfo.image.y, championInfo.image.w, championInfo.image.h);
                    imageCache[cacheKey] = iconBuffer;
                    return await (0, canvas_1.loadImage)(iconBuffer);
                }
            }
            catch (error) {
        ...
        const response = await axios_1.default.get(`https://ddragon.leagueoflegends.com/cdn/${CURRENT_PATCH}/img/champion/${standardChampName}.png`, { responseType: 'arraybuffer' });
        const buffer = Buffer.from(response.data);
        imageCache[cacheKey] = buffer;
        return await (0, canvas_1.loadImage)(buffer);
    }
        ...
        try {
            return await (0, canvas_1.loadImage)(path_1.default.join(__dirname, '../..', 'assets', 'champion_icons', `${championId}.png`));
        }
        catch (innerError) {
            // Last resort - create a blank icon
            const canvas = (0, canvas_1.createCanvas)(48, 48);
            const ctx = canvas.getContext('2d');
            ctx.fillStyle = '#1a1a2e';
            ctx.fillRect(0, 0, 48, 48);
            return await (0, canvas_1.loadImage)(canvas.toBuffer());
        }
    }
}
/**
 ...
 */
async function fetchTFTTraitIcon(traitId) {
    try {
        ...
        if (traitInfo && traitInfo.image) {
            try {
                ...
                try {
                    ...
                    const iconBuffer = extractIconFromSprite(spriteImage, traitInfo.image.x, traitInfo.image.y, traitInfo.image.w, traitInfo.image.h);
                    imageCache[cacheKey] = iconBuffer;
                    return await (0, canvas_1.loadImage)(iconBuffer);
                }
            }
        ...
        const response = await axios_1.default.get(`https://raw.communitydragon.org/latest/game/assets/ux/tft/traiticons/trait_icon_9_${cleanTraitName}.png`, { responseType: 'arraybuffer' });
        const buffer = Buffer.from(response.data);
        imageCache[cacheKey] = buffer;
        return await (0, canvas_1.loadImage)(buffer);
    }
        ...
        // Last resort - create a blank icon
        const canvas = (0, canvas_1.createCanvas)(18, 18);
        const ctx = canvas.getContext('2d');
        ctx.fillStyle = '#1a1a2e';
 ...
 */
async function generateImage(matchDetails, rankedData, summonerName) {
    try {
        ...
        const tauntMessage = (0, randomMessage_1.default)('tft');
 ...
 */
function drawPlacementCircle(ctx, placement, x, y) {
    const radius = 40;
 ...
 */
async function drawTraitIcons(ctx, traits, startX, y) {
    const iconSize = 30;
    const padding = 5;
 ...
 * @param {number} y Y position
 */
async function drawAugments(ctx, augments, centerX, y) {
    const iconSize = 30;
 ...
 */
function getTierBackgroundColor(style) {
    switch (style) {
 ...
 */
function formatAugmentName(augmentId) {
 ...
 */
async function fetchTFTAugmentIcon(augmentId) {
    try {
        ...
        try {
            ...
            const response = await axios_1.default.get(`https://raw.communitydragon.org/latest/game/assets/ux/tft/augments/${cleanId.toLowerCase()}.png`, { responseType: 'arraybuffer' });
            const buffer = Buffer.from(response.data);
            imageCache[cacheKey] = buffer;
            return await (0, canvas_1.loadImage)(buffer);
        }
            ...
            const canvas = (0, canvas_1.createCanvas)(30, 30);
            const ctx = canvas.getContext('2d');
 ...
 * @param {number} y Y center position
 */
function drawPlacementHexagon(ctx, placement, x, y) {
    const size = 35;
 ...
 */
async function drawCompactComp(ctx, units, startX, centerY) {
    const champSize = 60;
    const padding = 10;
 ...
 */
async function drawItemIconsBelow(ctx, items, champX, topY) {
    const itemSize = 20;
    const padding = 2;
    const champSize = 60;
 ...
 */
async function fetchTFTItemIcon(itemId) {
    try {
        ...
        try {
            ...
            try {
                ...
                const response = await axios_1.default.get(`https://raw.communitydragon.org/latest/game/assets/maps/particles/tft/item_icons/items/${itemId}.png`, { responseType: 'arraybuffer' });
                const buffer = Buffer.from(response.data);
                imageCache[cacheKey] = buffer;
                return await (0, canvas_1.loadImage)(buffer);
            }
                ...
                try {
                    const response = await axios_1.default.get(`https://ddragon.leagueoflegends.com/cdn/${CURRENT_PATCH}/img/item/${itemId}.png`, { responseType: 'arraybuffer' });
                    const buffer = Buffer.from(response.data);
                    imageCache[cacheKey] = buffer;
                    return await (0, canvas_1.loadImage)(buffer);
                }
        ...
        const canvas = (0, canvas_1.createCanvas)(20, 20);
        const ctx = canvas.getContext('2d');
        ctx.beginPath();
 ...
 */
function drawStarsAbove(ctx, tier, centerX, bottomY, color) {
 ...
 */
async function fetchTFTAsset(assetPath) {
    try {
        ...
        // Fetch the asset
        const response = await axios_1.default.get(`https://ddragon.leagueoflegends.com/cdn/${CURRENT_PATCH}/img/${assetPath}`, { responseType: 'arraybuffer' });
        const buffer = Buffer.from(response.data);
        imageCache[cacheKey] = buffer;
        return await (0, canvas_1.loadImage)(buffer);
    }
 ...
 */
function getDragonAPIChampionName(championId) {
...
function getPlacementColor(placement) {
    ...
    if (placement <= 4)
        return '#C0C0C0'; // Silver for 2nd-4th
