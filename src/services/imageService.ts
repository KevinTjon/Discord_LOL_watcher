 ...
 */
export async function generateImage(details: string[], rankedData: RankedData): Promise<Buffer> {
    ...
    try {
        ...
        try {
            ...
            const championIcon = await loadImage(`http://ddragon.leagueoflegends.com/cdn/13.24.1/img/champion/${details[4]}.png`);
            ...
            try {
                const championIcon = await loadImage(path.join(__dirname, '../..', 'assets', 'champion_icons', `${details[4]}.png`));
        ...
        const tauntMessage = getRandomMessage('league');
 ...
 */
export function drawCenteredText(ctx: CanvasRenderingContext2D, text: string, x: number, y: number, maxWidth: number): void {
    try {
